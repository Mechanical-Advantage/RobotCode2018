 ...
 * Robot Drive Train
 */
public class DriveTrainSlaveDisableTest extends Subsystem {
	...
	private DriveTrain.DriveGear currentGear;
...
//	private double motionProfileNotifierUpdateTime;
	...
	public DriveTrainSlaveDisableTest() {
...
//			wheelBaseWidth = 22.5; // 18
		...
		case ORIGINAL_ROBOT_2018:
			...
			switchGear(DriveTrain.DriveGear.HIGH);
			break;
		case EVERYBOT_2018:
			encoderType = FeedbackDevice.QuadEncoder;
			ticksPerRotation = 1440;
...
//		leftTalonMaster.configPeakOutputVoltage(+12.0f, -12.0f);
		...
		leftTalonMaster.follow(rightTalonMaster);
...
//	}
	...
	private TalonSRX getSlave(int slave) {
		switch (slave) {
		case 1:
			return rightTalonSlave;
		case 2:
			return leftTalonMaster;
		case 3:
			return leftTalonSlave;
		}
		return null;
	}
	...
	private void enableSlave(int slave) {
		...
		talonSlave.follow(rightTalonMaster);
	}
    ...
    public void driveInchesPerSec(double left, double right) {
    		int maxVelocity;
		if (RobotMap.robot != RobotType.ORIGINAL_ROBOT_2018 || currentGear == DriveTrain.DriveGear.LOW) {
    ...
    public void drive(double left, double right, boolean alwaysHighMaxVel) {
    		if (Robot.oi.getDriveEnabled() && currentControlMode == DriveControlMode.STANDARD_DRIVE) {
	    		...
	    		int maxVelocity;
	    		if (RobotMap.robot != RobotType.ORIGINAL_ROBOT_2018 || (currentGear == DriveTrain.DriveGear.LOW && !alwaysHighMaxVel)) {
	    			maxVelocity = RobotMap.maxVelocityLow;
	    		} else {
	    			maxVelocity = RobotMap.maxVelocityHigh;
	    		}
...
//	    			leftTalonMaster.set(ControlMode.Velocity, left);
    ...
    public void setPID(double p, double i, double d, double f, int iZone) {
    		int slot;
    		if (currentControlMode == DriveControlMode.STANDARD_DRIVE && currentGear == DriveTrain.DriveGear.LOW) {
    			slot = 0;
    		} else if (currentControlMode == DriveControlMode.STANDARD_DRIVE && currentGear == DriveTrain.DriveGear.HIGH) {
    			slot = 1;
    		} else {
    			slot = 2;
    		}
	...
	public void switchGear(DriveTrain.DriveGear gear) {
		if (RobotMap.robot == RobotType.ORIGINAL_ROBOT_2018 && (Robot.oi == null || Robot.oi.isShiftingEnabled())) {
			...
			case HIGH:
				...
				currentGear = DriveTrain.DriveGear.HIGH;
			...
			case LOW:
				...
				currentGear = DriveTrain.DriveGear.LOW;
	...
	public DriveTrain.DriveGear getCurrentGear() {
		if (RobotMap.robot == RobotType.ORIGINAL_ROBOT_2018) {
			return currentGear;
		} else {
			return DriveTrain.DriveGear.UNSUPPORTED;
		}
	}
